    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-credentails', toolName: 'docker'){
                        sh 'docker build -t retail-app:v1.0.0 .'
                        sh 'docker tag retail-app:v1.0.0 iamsubbu3/retail-app:v1.0.0'
                        sh 'docker push iamsubbu3/retail-app:v1.0.0'
                    }
                }
            }
        }
        ...
        stage ('deploy to eks') {
            steps {
                script {
                    dir('k8s-manifests') {
                        withKubeConfig(caCertificate: ", clusterName: ", credentialsId: 'kubeconfig-file', namespace: ", restrictKubeConfigAccess: false, serverUrl: ") {
